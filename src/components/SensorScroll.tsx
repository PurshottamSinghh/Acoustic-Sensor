 ...
 *
 * Architecture (fixed-position approach — no sticky needed):
 *  - A FIXED full-screen canvas sits behind everything
 *  - A tall scrollable spacer div (400vh) drives the scroll progress
 *  - useScroll on the spacer maps scroll → frame index
 *  - Text sections are absolutely positioned within the spacer,
 *    but their opacity is derived from scroll progress
 */
export default function SensorScroll() {
  ...
  return (
    <>
      {/* ═══ FIXED canvas — always fills the viewport ═══ */}
      <div
        style={{
          position: 'fixed',
          top: 0,
...
/* ═══════════════════════════════════════════════════════════════
   TextOverlays — each section is fixed-positioned via
   `position: fixed` + controlled opacity from scroll progress.
   ═══════════════════════════════════════════════════════════════ */
...
function TextOverlays({ scrollYProgress }: { scrollYProgress: MotionValue<number> }) {
  ...
  return (
    <>
      ...
      {/* ── 60%: Piezoelectric Precision (Right) ─────────── */}
      ...
      <motion.div style={{ ...fixedFull, opacity: o4, y: y4, display: 'flex', flexDirection: 'column' as const, alignItems: 'center', justifyContent: 'center', textAlign: 'center' as const, padding: '0 1.5rem' }}>
        ...
        <button
          ...
          onMouseEnter={(e) => {
            ...
            e.currentTarget.style.boxShadow = '0 0 30px rgba(110,231,183,0.1)';
          }}
          onMouseLeave={(e) => {
